...
<meta charset="UTF-8">
<title>Find Multi-Stop Flights</title>
</head>
<style>
...
//connect to a DB and try to make a drop down table of all the contries and airports to start a flight from
	try {
		...
		//Get the database connection
			...
			out.print("<form method=\"post\" action=\"CRSearchMultiStopFlights.jsp\">");
			...
			while (result.next()) {
				...
				out.print( "<option value= \"" + CountryDepartID + "\">"+ CountryOfDepart + "</option>" );
			}
			out.print("</select> ");
			...
			while (result.next()) {
				...
				out.print( "<option value= \"" + CountryArrivalID + "\">"+ CountryOfArrival + "</option> " );
			}
			out.print("</select>");
			
			out.print("Departure date:  <input type=\"date\" name=\"departDate\" /input> <small> Format: YYYY-MM-DD </small>");
			out.print("<input type = \"hidden\" value = \""+request.getParameter("CustomerToHelp")+"\" name = \"CustomerToHelp\">");
